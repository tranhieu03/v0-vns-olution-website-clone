...
export function Problems() {
  return (
    <section className="py-16 lg:py-24 bg-[#FAFDFA] relative overflow-hidden">
      <div className="container relative mx-auto px-4 lg:px-8">
        <div className="mx-auto max-w-2xl text-center mb-14">
          <p className="text-sm font-semibold text-[#00A86B] mb-3 uppercase tracking-wide">Thách thức phổ biến</p>
        ...
        <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
          {challenges.map((challenge, index) => (
            <div
              key={index}
              className={`group rounded-2xl border border-slate-200 bg-white p-6 transition-smooth hover-lift hover:border-[#00A86B]/30 hover:shadow-lg opacity-0 animate-fade-up stagger-${index + 1}`}
              style={{ animationFillMode: 'forwards' }}
            >
              <div className="flex h-12 w-12 items-center justify-center rounded-xl bg-red-50 text-red-500 transition-all group-hover:bg-[#00A86B]/10 group-hover:text-[#00A86B]">
                <challenge.icon className="h-6 w-6" />
              </div>
              <h3 className="mt-5 text-lg font-semibold text-[#002B05]">
                {challenge.title}
              </h3>
